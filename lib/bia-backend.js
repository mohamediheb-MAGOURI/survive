...
export function gapView(item) {
  const currentLevel = Number(item.currentValue || 0)
  const expectedLevel = Number(item.targetValue || 0)
  return {
    id: item.id,
    ...
    currentLevel,
    expectedLevel,
    risks: String(item.description || '').split('\n').filter(Boolean),
    recommendations: String(item.recommendation || '').split('\n').filter(Boolean),
  }
}
